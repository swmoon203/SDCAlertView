...
class ActionsCollectionView: UICollectionView {
    ...
    init() {
        ...
        let nibName = NSStringFromClass(ActionCell).components(separatedBy: ".").last!
    ...
    func highlightAction(forPanGesture sender: UIGestureRecognizer) {
        ...
        guard let indexPath = self.indexPathForItem(at: touchPoint),
              ...
              , cell != self.highlightedCell && self.actions[(indexPath as NSIndexPath).item].enabled else
        {
            return
        }
...
extension ActionsCollectionView: UICollectionViewDataSource {
        ...
        -> UICollectionViewCell
    {
        ...
        cell?.setAction(self.actions[(indexPath as NSIndexPath).item], withVisualStyle: self.visualStyle)
        return cell!
    }
}
